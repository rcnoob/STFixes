...
{
    private TriggerPushTouchHandler(string name, Detour preDetour, ILogger<STFixes> logger)
        : base(name, preDetour, logger) {}
    
    ~TriggerPushTouchHandler() { Stop(); }
    
    // Gets called in BaseHandler by public static T Build<T>(ILogger<STFixes> logger) where T : BaseHandler
    public static TriggerPushTouchHandler Build(ILogger<STFixes> logger)
    {
